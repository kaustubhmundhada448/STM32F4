...
// Return value:
// = SD_PRESENT when media is inserted
// SD_NOT_PRESENT = when no media is inserted
// ------------------------------------------------ --------------
uint8_t UB_SDCard_CheckMedia(void)
{
  __IO uint8_t status = SD_NOT_PRESENT;

  // Query whether SD card is inserted
  status=SD_Detect();
...
// ------------------------------------------------ --------------
// Init the SD card
// -> The rest of the initialization (GPIO NVIC)
